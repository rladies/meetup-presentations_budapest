...
data[, delay_diff := arr_delay-dep_delay][, max(delay_diff, na.rm=TRUE)]
...
data[air_time>0, .N, by=.(origin, dest)][order(-N)]
#------------------------------------
...
data[, AvgAirTime:=mean(air_time, na.rm=TRUE), by=.(origin, dest)][air_time>1.5*AvgAirTime][order(-arr_delay)][, .(origin, dest, air_time, AvgAirTime, arr_delay)]
#------------------------------------
