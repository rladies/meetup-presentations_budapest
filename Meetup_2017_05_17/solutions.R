...
adat %>%
  filter(IskolaTipusa %in% c("Gimn4Osztalyos", "Gimn6Osztalyos", "Gimn8Osztalyos"))
...
szovegertes <- legjobb_mat$PontSzovegertes
termtud <- legjobb_mat$PontTermTud

## 6.5 Nyerjuk ki a tobbiek pontszamait egyeb teruletekrol!
szovegertes_gim <- adat %>%
  filter(IskolaTipusa %in% c("Gimn4Osztalyos", "Gimn6Osztalyos", "Gimn8Osztalyos")) %>%
  select(PontSzovegertes)
termtud_gim <- adat %>%
  filter(IskolaTipusa %in% c("Gimn4Osztalyos", "Gimn6Osztalyos", "Gimn8Osztalyos")) %>%
  select(PontTermTud)
...
ecdf(szovegertes_gim$PontSzovegertes)(szovegertes)
ecdf(termtud_gim$PontTermTud)(termtud)
...
ggplot(gimn) +
  geom_point(aes(x = PontMat, y = PontTermTud))
...
adat %>%
  filter(IskolaTipusa == "Szakmunkaskepzo") %>%
  group_by(IskolaTipusa) %>%
  summarise(medianSzoveg = median(PontSzovegertes),
            medianTermtud = median(PontTermTud))
