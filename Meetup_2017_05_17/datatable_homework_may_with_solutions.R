...
## A helyes megoldásokhoz többféle úton is el lehet jutni, de kérlem, hogy 
...
## értékekkel:
## indulas = "torolt", ha arr_time hiányzik
...
data_flights[is.na(arr_time), indulas := "torolt"]
data_flights[dep_delay>10, indulas := "keses"]
data_flights[is.na(indulas), indulas := "idoben"]
data_flights <- data_flights[, .(origin, time_hour, indulas)]

data_weather[precip==0, csapadek:= "nincs_csapadek"]
data_weather[precip>0 & precip<0.2 , csapadek:= "keves_csapadek"]
data_weather[precip>=0.2, csapadek:= "sok_csapadek"]
data_weather <- data_weather[, .(origin, time_hour, csapadek)]
...
##
## B) Az időjárás adatok minden órára adottak, de vannak olyan órák, amelyekben 
## nem volt tervezve repülőjárat indulás. Ezeket az adatokat szűrjük ki az összekapcsolt
## táblából! (pl !is.na(indulas) kifejezés használatával)
## Az eredményt mentsük el egy flights_with_weather nevű data.table-be!

setkey(data_flights, origin, time_hour)
setkey(data_weather, origin, time_hour)

flights_with_weather <- data_flights[data_weather][!is.na(indulas)]
...
flights_with_weather[, count:=1]
dcasted <- dcast(flights_with_weather, csapadek ~ indulas, value.var="count", fun.aggregate = sum)

dcasted[, total:= idoben+keses+torolt ]
dcasted[, torolt_arany:= 100*torolt/total ]
dcasted[, keses_arany:= 100*keses/total ]
...
##
## Bónusz kérdés:
## Melyik időjárási körülmény befolyásolja jobban a felszállást, a csapadék vagy a szél?


data_flights <- data.table(flights)
data_weather <- data.table(weather)

## Flights előkészítése
data_flights[is.na(arr_time), indulas := "torolt"]
data_flights[dep_delay>10, indulas := "keses"]
data_flights[is.na(indulas), indulas := "idoben"]
data_flights <- data_flights[, .(origin, time_hour, indulas)]
...
## Szélerő kategóriák
data_weather[wind_speed<7, szel:= "nincs_szel"]
data_weather[wind_speed>=7 & wind_speed<12 , szel:= "gyenge_szel"]
data_weather[wind_speed>=12, szel:= "eros_szel"]
data_weather <- data_weather[, .(origin, time_hour, szel)]

# Összekapcsolás
setkey(data_flights, origin, time_hour)
flights_with_wind <- data_flights[data_weather][!is.na(indulas)]

# Dcast
flights_with_wind[,count:=1]
dcasted2 <- dcast(flights_with_wind, szel ~ indulas, value.var="count", fun.aggregate = sum)

dcasted2[, total:= idoben+keses+torolt ]
dcasted2[, torolt_arany:= 100*torolt/total ]
dcasted2[, keses_arany:= 100*keses/total ]
